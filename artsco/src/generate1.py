...
def main(args):
    ...
    for batch in progress:
        ...
        # Build local results
        for p, c, pc, pt, vv, vt in zip(prompts, completions, player_candidates, player_thinks, voter_votes, voter_thinks):
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--results_path_root", type=str, default="artsco/data")
    p.add_argument("--results_path", type=str, default="")

    p.add_argument("--per_device_batch", type=int, default=128)
    ...
    for task in TASKS:
        for split in SPLITS:
            for model_name in MODELS:
                ...
                run_args.results_path  = os.path.join(args.results_path_root,  run_args.task, run_args.model_name,  f"{run_args.split}_step1.json")
